...
class SchedulerStatusRepositoryImplementation(SchedulerStatusRepository):
    ...
    def get_scheduler_status(self, scheduler_names, env) -> Tuple[Exception, List[SchedulerStatusValueObject]]:
        ...
        schedulers = {
            'helper-auto-release-scheduler': {'time': '24h'},
            'helper-contract-scheduler': {'time': '24h'},
            'helper-nra-scheduler': {'time': '1h'},
            'helper-usage-scheduler': {'time': '24h'},
        }
        ...
        try:
            with ThreadPoolExecutor() as executor:
                for scheduler_name in target_scheduler_names:
                    query = 'floor(increase({metric_name}{{env="{env}",scheduler_name="{scheduler_name}", status="Succeeded"}}[{time}]))'.format(
        ...
        for metrics_object in prometheus_metrics_list:
            ...
            if int(execution_success_times) >= 1:
                ...
                status = 'Unknown'
            metrics_list.append(
                SchedulerStatusValueObject(name=metrics_object['metric']['scheduler_name'], status=status)
            )
