...
@click.option(
    "--system-names",
    "-s",
    help="System names to check, string separated by comma, default: all systems",
)
@click.option("--cluster", "-c", help="Cluster to check system health", required=True)
...
def system_health_command(
    ...
    error, system_healths = system_health_application.get_system_health(
        system_names=system_name_list, cluster=cluster, env=env
    )
