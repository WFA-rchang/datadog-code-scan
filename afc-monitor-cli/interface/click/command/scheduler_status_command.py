...
def scheduler_status_command(scheduler_status_application: SchedulerStatusApplicationImplementation = Provide[Container.scheduler_status_application],
    ...
    if scheduler_names != None:
        scheduler_names_list = scheduler_names.split(',')

    error, scheduler_status = scheduler_status_application.get_scheduler_status(scheduler_names=scheduler_names_list, env=env)
    ...
    for scheduler in scheduler_status:
        click.echo(
            f"Scheduler Name: {scheduler.name}, Status: {scheduler.status}"
        )
